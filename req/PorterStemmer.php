...
class PorterStemmer {
    ...
    private static function step1ab($word) {
        // Part a
        if (substr($word, -1) == 's' && self::is_in_special_list($word)==false) {

            self::replace($word, 'sses', 'ss') OR self::replace($word, 'ies', 'i') OR self::replace($word, 'ss', 'ss') OR self::replace($word, 's', '');
        }

        // Part b
        if ((substr($word, -2, 1) != 'e' OR !self::replace($word, 'eed', 'ee', 0)) && self::is_in_special_list($word)==false) { // First rule
            $v = self::$regex_vowel;
    ...
    private static function step1c($word) {
        ...
        if (substr($word, -1) == 'y' && preg_match("#$v+#", substr($word, 0, -1)) && self::is_in_special_list($word) == false) {
    ...
    private static function step2($word) {
        if (self::is_in_special_list($word) == false) {
    ...
    private static function step5($word) {
        // Part a
        if (substr($word, -1) == 'e' && self::is_in_special_list($word) == false) {
...
//                self::replace($word, 'e', '');
    ...
    /**
     * Compares the word with an array of special words as pre-defined by the user.
     *
     * @param string $word  String to check
     * @return bool         Whether the $word string was in the array of special words or not
     * 
     */
    private static function is_in_special_list($word) {
        $array = array("january", "february", "may", "june", "july", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday","decide", "google","cadbury","dairy","awesome");
